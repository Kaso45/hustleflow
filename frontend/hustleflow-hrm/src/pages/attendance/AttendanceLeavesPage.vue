<template>
  <div class="page-container">
    ...
    <div class="table-scroll-wrapper">
      ...
      -->
      <FloatingTable :gridColumns="gridLayout" class="fixed-min-width">
        <template #header>
          <div>Employee</div>
          <div>Type</div>
          <div>Duration</div>
        ...
        <div v-else-if="filteredLeaves.length === 0" class="state-msg">
          No requests found.
        </div>
        ...
        >
          ...
          <div class="col-info">
            ...
            <div
              v-else
              class="w-[38px] h-[38px] bg-gray-100 rounded-full flex-shrink-0"
            ></div>
            ...
            <div class="info-text">
              <div
                class="emp-name"
                v-if="getEmployee(req.employeeId)"
                :title="getEmployee(req.employeeId).name"
              >
          ...
          <div>
            <span class="type-badge">{{
              humanizeLeaveType(req.leaveType)
            }}</span>
          </div>

          <!-- Col 3: Duration -->
          <div class="duration-text text-gray-600">
            <div class="font-medium text-[#0b2433] whitespace-nowrap">
              {{ formatDate(req.startDate) }}
            </div>
            <div class="text-gray-400 mt-0.5 whitespace-nowrap">
          ...
          <!-- Col 4: Reason -->
          <div
            class="reason-text text-gray-500 truncate-2-lines"
            :title="req.reason"
          >
            {{ req.reason || "--" }}
          </div>
          ...
          <div class="actions-group">
            <template v-if="req.status === 'PENDING'">
              <button
                ...
                @click.stop="handleStatusChange(req.id, 'APPROVED')"
                title="Approve"
              >
                <Check class="icon-scale" />
                <span class="btn-text">Approve</span>
              </button>
              <button
                class="action-btn reject"
                @click.stop="handleStatusChange(req.id, 'REJECTED')"
                title="Reject"
              >
            ...
            <span
              v-else
              class="processed-text text-gray-400 italic font-medium"
            >
              Processed
            </span>
          </div>
...
.type-badge {
  ...
  font-weight: 600;
  background: #f1f5f9;
  padding: clamp(2px, 0.4vw, 4px) clamp(6px, 0.8vw, 8px);
  border-radius: 6px;
  color: #475569;
  white-space: nowrap;
}
