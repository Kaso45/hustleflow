  ...
  <div class="page-container">
    ...
    <div class="filter-bar">
      ...
      <div class="filter-scroll">
        <button
          class="filter-pill"
          :class="{ active: !selectedEmpId }"
          @click="selectedEmpId = null"
        >
          All Employees
        </button>
        <button
      ...
      >
        ...
        <div class="col-info">
          <div class="mini-assignee">
            <BaseAvatar
              :name="getEmployee(task.assigneeId)?.name || '?'"
              :size="24"
            />
            <span class="assignee-name">{{
              getEmployee(task.assigneeId)?.name || "Unassigned"
            }}</span>
          </div>
        </div>
        <div class="project-tag-wrapper">
          <span class="project-tag">{{
            getProject(task.projectId)?.projectName || "General"
          }}</span>
        </div>
        <div class="date-cell">
      ...
      <div class="notion-modal-content">
        ...
        <div class="modal-body-scroll">
            ...
            <div class="prop-row">
              ...
              <div class="prop-value">
                <select
                  ...
                  class="prop-select-status"
                  :class="modalData.status"
                >
          ...
          <div class="note-section">
            ...
            <textarea
              v-model="modalData.completionNote"
              class="note-textarea"
              placeholder="Remarks..."
            ></textarea>
          </div>
...
const hiddenTasks = ref(
  JSON.parse(localStorage.getItem("hidden_tasks") || "[]")
);
...
const handleSave = async () => {
  try {
    if (isEditing.value) {
      const res = await taskService.updateTask(modalData.id, {
        status: modalData.status,
        completionNote: modalData.completionNote,
      });
...
const filteredTasks = computed(() => {
  ...
  if (selectedEmpId.value)
    res = res.filter((t) => t.assigneeId === selectedEmpId.value);
...
/* Table styles */
.row-card {
  grid-template-columns: 2fr 1.2fr 1.2fr 1fr 1fr 90px;
  gap: 16px;
}
